...
// ─── Temporary guest mock-session helpers ───────────────────────────────────

const GUEST_SESSION_KEY = 'guest_mock_session_id';
const PENDING_MOCK_RESULT_KEY = 'pending_mock_result';
const TRANSFERRED_MOCK_RESULT_KEY = 'transferred_mock_result';
...
export function getPendingMockResult() {
  if (typeof window === 'undefined') return null;
  const value = window.sessionStorage.getItem(PENDING_MOCK_RESULT_KEY);
...
export function setPendingMockResult(result) {
  if (typeof window === 'undefined') return;
  window.sessionStorage.setItem(PENDING_MOCK_RESULT_KEY, JSON.stringify(result));
}

export function clearPendingMockResult() {
  if (typeof window === 'undefined') return;
  window.sessionStorage.removeItem(PENDING_MOCK_RESULT_KEY);
}

export function getTransferredMockResult() {
  if (typeof window === 'undefined') return null;
  const value = window.sessionStorage.getItem(TRANSFERRED_MOCK_RESULT_KEY);
  if (!value) return null;
  try {
    return JSON.parse(value);
  } catch {
...
export function setTransferredMockResult(result) {
  if (typeof window === 'undefined') return;
  window.sessionStorage.setItem(TRANSFERRED_MOCK_RESULT_KEY, JSON.stringify(result));
}

export function clearTransferredMockResult() {
  if (typeof window === 'undefined') return;
  window.sessionStorage.removeItem(TRANSFERRED_MOCK_RESULT_KEY);
}
